...
describe('CushionManager (ConfigManager)', () => {
  ...
  describe('Rule Management', () => {
    ...
    it('should clear all rules', () => {
      ...
      manager.setCushion('/api/comments/*', { mapping: { text: 'comment_text' } });
  ...
  describe('Complex Pattern Scenarios', () => {
    it('should handle multiple pattern types together', () => {
      ...
      manager.setCushion('/api/v2/user/:id', { mapping: { version: 'v2', type: 'user' } });

      expect(manager.getCushion('/api/v1/posts')).toEqual({ mapping: { version: 'v1' } });
      expect(manager.getCushion('/api/v2/posts')).toEqual({ mapping: { version: 'v2' } });
  ...
  describe('Real-world Configuration Scenarios', () => {
    it('should handle 단계적 API 버전 마이그레이션', () => {
      ...
      // 각 버전별로 다른 매핑 적용
      expect(manager.getCushion('/api/v1/user')?.mapping.name).toBe('user_name');
    ...
    it('should handle A/B 테스트 시나리오', () => {
      ...
      // 베타 그룹은 새로운 규칙
      expect(manager.getCushion('/api/user?group=beta')?.mapping.name).toBe('userName');
    });
  });
});